...
uup=0.35;
x0=0.1;
int[n_,m_,p_,q_]:=If[(n<0)\[Or](m<0)\[Or](p<0)\[Or](q<0),0,If[EvenQ[n+m+p+q],((Sqrt[m!] Sqrt[n!])/(Sqrt[2] \[Pi]^2 Sqrt[p!] Sqrt[q!]))*Sum[(Gamma[1/2 (1+m+n+p-q-2 t)] Gamma[1/2 (1+m+n-p+q-2 t)] Gamma[1/2 (1-m-n+p+q)+t])/( (m-t)! (n-t)! t!),{t,0,Min[n,m]}],0]];Block[{
hamKin=Total[(
...
hamInt=Total[( 
-int[#1-1,#2-1,#3-1,#4-1](vEm[#1,#2+sites]hEm[#3+sites,#4]+hEm[#1,#2+sites]vEm[#3+sites,#4])
